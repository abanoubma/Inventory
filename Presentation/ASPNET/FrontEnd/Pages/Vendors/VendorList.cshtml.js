...
const App = {
    setup() {
        ...
        const services = {
            ...
            createMainData: async (name, description, phoneNumber, faxNumber, createdById) => {
                try {
                    const response = await AxiosManager.post('/Vendor/CreateVendor', {
                        name, description, phoneNumber, faxNumber, createdById
                    });
                    return response;
                } catch (error) {
                    throw error;
            ...
            updateMainData: async (id, name, description, phoneNumber, faxNumber, updatedById) => {
                try {
                    const response = await AxiosManager.post('/Vendor/UpdateVendor', {
                        id, name, description, phoneNumber, faxNumber, updatedById
                    });
                    return response;
                } catch (error) {
                    throw error;
        ...
        const handler = {
            handleSubmit: async function () {
                try {
                    ...
                    const response = state.id === ''
                        ? await services.createMainData(state.name, state.description, state.phoneNumber, state.faxNumber, StorageManager.getUserId())
                        : state.deleteMode
                            ? await services.deleteMainData(state.id, StorageManager.getUserId())
                            : await services.updateMainData(state.id, state.name, state.description, state.phoneNumber, state.faxNumber, StorageManager.getUserId());
            ...
            create: async (dataSource) => {
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    columns: [
                        ...
                        { field: 'number', headerText: 'Number', width: 150, minWidth: 150 },
                        { field: 'name', headerText: 'Name', width: 200, minWidth: 200 },
                        { field: 'description', headerText: 'Description', width: 200, minWidth: 200 },
                        { field: 'phoneNumber', headerText: 'Phone', width: 200, minWidth: 200 },
                        { field: 'faxNumber', headerText: 'Fax', width: 200, minWidth: 200 },
                        { field: 'createdAtUtc', headerText: 'Created At UTC', width: 150, format: 'yyyy-MM-dd HH:mm' }
                    ],
                    toolbar: [
                        'ExcelExport', 'Search',
                    ...
                    dataBound: function () {
                        ...
                        mainGrid.obj.autoFitColumns(['name', 'description', 'phoneNumber', 'faxNumber', 'createdAtUtc']);
                    },
                    excelExportComplete: () => { },
                    rowSelected: () => {
                        if (mainGrid.obj.getSelectedRecords().length == 1) {
